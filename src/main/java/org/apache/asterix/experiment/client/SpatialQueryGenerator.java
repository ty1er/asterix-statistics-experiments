...
public class SpatialQueryGenerator {
    ...
    public void start() throws Exception {
        ...
        threadPool.submit(new QueryGenerator(sem, restHost, restPort, orchHost, orchPort, partitionRangeStart, duration,
                openStreetMapFilePath, isIndexOnlyPlan));
        sem.acquire();
    }
    ...
    public static class QueryGenerator implements Runnable {
        ...
        public void run() {
            ...
            try {
                outputFilePath = openStreetMapFilePath.substring(0, openStreetMapFilePath.lastIndexOf(File.separator))
                        + File.separator + "QueryGenResult-" + InetAddress.getLocalHost().getHostAddress() + ".txt";
        ...
        private String getRangeQueryAQL(float radius, float x, float y, boolean isIndexOnlyPlan) {
            ...
            if (isIndexOnlyPlan) {
                ...
                sb.append(
                        "where spatial-intersect($x.sender-location, $n) and $x.btree-extra-field1 <= int32(\"2147483647\") ");
            }
            sb.append("return $x ");
            sb.append(");");
            return sb.toString();
